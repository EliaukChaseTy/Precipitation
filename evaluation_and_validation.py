...
def import_feature_data_verification(startdate, enddate, exclude_MM, exclude_hh, perc_exclude,
    ...
    for n in range(0, int((enddate - startdate).total_seconds()/(15*60))):
        ...
        try:
            files.append(glob.glob('/gws/nopw/j04/swift/random_forest_precip/1_training_data/'+YYYY+'/'+MM+'/'+YYYY+MM+DD+hh+mm+'.pkl')[0])
        except:
            continue
...
ver = add_CRR(ver_arr = ver, bin_edges=np.array(RF_parameters["bin_edges"]).astype(np.float64), CRR_dir = '/gws/nopw/j04/swift/random_forest_precip/4_verify_model/CRR_regridded/data/')

ver = add_CRR_Ph(ver_arr = ver, bin_edges=np.array(RF_parameters["bin_edges"]).astype(np.float64), CRR_Ph_dir = '/gws/nopw/j04/swift/random_forest_precip/4_verify_model/CRR-Ph_regridded/data/')
